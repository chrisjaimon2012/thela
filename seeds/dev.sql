...
UPDATE setting SET value = 'Maharashtra'             WHERE key = 'shipping.allowed_states';
UPDATE setting SET value = '411001'                  WHERE key = 'shipping.origin_pincode';

-- Six blanks: three sizes x two frame colours. Small is made to order
-- (tracked = 0) to exercise both paths.
INSERT INTO stock (sku, label, on_hand, reserved, tracked) VALUES
...
-- Dimensions are the PACKED carton, not the frame: Indian couriers bill on
-- max(dead weight, L*W*H/5000), so a light, bulky parcel is priced on its box.
INSERT INTO variant
  (id, product_id, sku, option_1, option_2, price_paise, weight_g, len_mm, wid_mm, hgt_mm, position) VALUES
